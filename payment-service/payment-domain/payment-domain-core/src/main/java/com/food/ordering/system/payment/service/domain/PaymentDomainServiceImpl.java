...
import static com.food.ordering.system.domain.DomainConstants.UTC_PLUS_4;
...
public class PaymentDomainServiceImpl implements PaymentDomainService {
    ...
    public PaymentEvent validateAndInitiatePayment(Payment payment, CreditEntry creditEntry,
        ...
        if (failureMessages.isEmpty()) {
            ...
            return new PaymentCompletedEvent(payment, ZonedDateTime.now(ZoneId.of(UTC_PLUS_4)), paymentCompletedEventDomainEventPublisher);
        } else {
            log.info("Payment initiation is failed for order with id: {}", payment.getOrderId().getValue());
            payment.updateStatus(PaymentStatus.FAILED);
            return new PaymentFailedEvent(payment, ZonedDateTime.now(ZoneId.of(UTC_PLUS_4)), failureMessages, paymentFailedEventDomainEventPublisher);
        }
    }
    ...
    public PaymentEvent validateAndCancelPayment(Payment payment, CreditEntry creditEntry, List<CreditHistory> creditHistories, List<String> failureMessages, DomainEventPublisher<PaymentCancelledEvent> paymentCancelledEventDomainEventPublisher, DomainEventPublisher<PaymentFailedEvent> paymentFailedEventDomainEventPublisher) {
        ...
        if (failureMessages.isEmpty()) {
            ...
            return new PaymentCancelledEvent(payment, ZonedDateTime.now(ZoneId.of(UTC_PLUS_4)), paymentCancelledEventDomainEventPublisher);
        } else {
            log.info("Payment cancellation is failed for order with id: {}", payment.getOrderId().getValue());
            payment.updateStatus(PaymentStatus.FAILED);
            return new PaymentFailedEvent(payment, ZonedDateTime.now(ZoneId.of(UTC_PLUS_4)), failureMessages, paymentFailedEventDomainEventPublisher);
        }
    }
